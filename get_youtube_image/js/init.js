...
$(function() {
	...
	$(".replace_url").on("click", function (event) {
		$("#youtube_image").html('<img src="'+$(this).attr("href")+'">');

    //prevent the default action for the click event
    event.preventDefault();

    //get the top offset of the target anchor
    var target_offset = $("#banner").offset();
	...
	$("#submit").on("click", function () {
		if($('#youtubecode').val()==''){
			alert('Please Press Youtube Code.');
		}
		else {
			var $url = "http://img.youtube.com/vi/"+$('#youtubecode').val();
			$(".replace_url").each(function(){
				$(this).attr("href", $url+"/"+$(this).attr("data-value")+".jpg");
			});
			$(".timestamp").each(function(){
				$(this).html($url+"/"+$(this).attr("data-value")+".jpg");
			});
